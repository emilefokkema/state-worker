describe('a state worker that cancels queries gracefully', () => {
    ...
    beforeAll(async () => {
        worker = await StateWorker.create({
            path: 'base/test/web/examples/expensive-calculation-example.js',
    ...
    it('should handle a number of queries and a numer of commands and a number of queries', async () => {
        ...
        await expectAsync(firstQueryPromise).toBeRejectedWithError('execution was cancelled');
        await expectAsync(secondQueryPromise).toBeRejectedWithError('execution was cancelled');
        await expectAsync(thirdQueryPromise).toBeRejectedWithError('execution was cancelled');
